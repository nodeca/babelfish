...
// used below in varify
var stoper = '<~_#@!', stoperRE = /<~_#@!/g;
...
function varify(phrase, params) {
  ...
  // 1. Replace \#{ escaped case with sequence, that will never happen.
...
function compile(str, locale) {
  ...
  forEach(nodes, function (node) {
    ...
    if ('plural' === node.type) {
      translator.push(format(
        'str += (+(%s) != (%s)) ? ("[invalid plurals amount: %s(" + String(%s) + ")]") : this.varify(this.pluralize("%s", +%s, %j), params);',
