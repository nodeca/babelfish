    ...
    if ('plural' === node.type) {
      translator.push(
        "str += (!" + anchor + " || isNaN(" + anchor + ")) " +
          "? '<invalid_amount>' " +
          ": this.pluralize(+" + anchor + ", " + JSON.stringify(node.forms) + ");"
      );
      return;
    }
...
// reload pluralizers cache
function reloadPluralizer(self, locale) {
  var fallbacks = (self._fallbacks[locale] || []).slice(), lang;

  while (fallbacks.length) {
    lang = fallbacks.shift().split('-', 1).shift();
    if (Pluralizer[lang]) {
      self._pluralizers[locale] = Pluralizer[lang];
      return;
    }
  }

  self._pluralizers[locale] = Pluralizer[GENERIC_LOCALE];
}
...
BabelFish.prototype.getPluralizer = function getPluralizer(locale) {
  if (!this._pluralizers[locale]) {
    reloadPluralizer(this, locale);
  }

  return this._pluralizers[locale];
};
