...
var Rules = require('./babelfish/rules');
...
function compile(str) {
  var parts = Parser.parse(str);

  if (1 === parts.length && 'string' === parts[0].type) {
    return parts[0].value;
  }

  return function translator(params) {
    var self = this, str = '';

    params = Common.flattenParams(params);
    ...
    parts.forEach(function (part) {
      ...
      if ('variable' === part.type) {
        str += params[part.anchor].toString();
        return;
      }

      if ('pluralizer' === part.type) {
        str += self.pluralize(params[part.anchor], params.value);
        return;
      }
...
// Iterator for Common.filter to leave non-scopes only
...
function mergeTranslations(receiver, transmitter, locale) {
  Common.each(transmitter, function (data, key) {
    // go deeper
    if ('object' === data.type) {
    ...
    if (data.locale === locale) {
      receiver[key] = Common.merge({}, data);
    }
  });
...
BabelFish.prototype.addPhrase = function addPhrase(locale, scope, value) {
  ...
  if ('string' !== t && 'object' !== t && value !== (value || '').toString()) {
    ...
    // recursive recursion
    Common.each(value, function (value, key) {
...
BabelFish.prototype.translate = function translate(locale, scope, params) {
  ...
  case 'function':  return translator.value.apply(this.getRules(locale), params);
...
//  internal
//  BabelFish#getRules(locale) -> Object
//
//  Returns object with pluralizer and other localizators for given locale.
BabelFish.prototype.getRules = function getRules(locale) {
  // TODO: respect given locale fallbacks while choosing rules
  return Rules[locale] || Rules[this.defaultLocale] || Rules[GENERIC_LOCALE];
};
...
BabelFish.prototype.getCompiledData = function getCompiledData(locale, scope, options) {
  ...
  return {
    type: 'object',
    locale: null,
    value: Common.filter(storage[parts.key].value, flatStorageFilter)
  };
};
