...
function compile(str) {
  ...
  if (1 === nodes.length && 'scalar' === nodes[0].type) {
  ...
  return function translator(params) {
    ...
    nodes.forEach(function (node) {
      ...
      if ('variable' === node.type) {
        str += (params[node.anchor] || '<undefined>').toString();
        return;
      }

      if ('pluralizer' === node.type) {
        str += self.pluralize(+params[node.anchor], node.forms);
        return;
      }
...
BabelFish.prototype.translate = function translate(locale, scope, params) {
  ...
  case 'function':  return translator.value.call(this.getRules(locale), params);
