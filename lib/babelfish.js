...
function BabelFish(defaultLocale) {
  this._defautLocale = defaultLocale || 'en';
  this._fallback = {};
  this._storage = {};
}
...
function flatenParams(obj) {
  ...
  common.each(obj, function (val, key) {
    ...
    if ('object' === typeof obj[key]) {
      common.each(obj[key], function (val, sub_key) {
        params[key + '.' + sub_key] = val;
      });
    }
...
BabelFish.prototype.setLocaleFallback = function setLocaleFallback(locale, fallback) {
  this._fallback[locale] = fallback || this._defautLocale;
  return this;
};
...
BabelFish.prototype.getLocaleFallback = function getLocaleFallback(locale) {
  ...
  if (locale === this._defautLocale) {
    return null;
  }

  return this._fallback[locale] || this._defautLocale;
};
