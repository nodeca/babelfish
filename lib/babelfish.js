...
// Returns fallbacks of given locale
function getFallbacks(self, locale) {
  var fallbacks = self._fallbacks[locale];

  if (!fallbacks) {
    return [self.defaultLocale];
  }

  return fallbacks.slice().concat([self.defaultLocale]);
}
...
function proposeTranslationFallback(self, fallback, scope, data) {
  // not implemented yet
}
...
function recompile(self, locale) {
  var fallbacks, fb_locale, old_storage, new_storage, len;

  fallbacks = getFallbacks(self, locale).slice(); // get copy of fallbacks
  old_storage = getLocaleStorage(self, locale);
  new_storage = Common.merge({}, getLocaleStorage(self, fallbacks.pop()));
 ...
 *
 *  **WARNING** calling this method force recompilation of translations for
 *  given `locale` in order to respect new fallbacks. Consider calling this
 *  method before you'll populate BabelFish with phrases.
 *
 *  ##### Errors
 *
...
BabelFish.prototype.setFallback = function setFallback(locale, fallbacks) {
  ...
  fallbacks.concat([self.defaultLocale]).forEach(function (fallback) {
    if (!self._fallbacksReverse[fallback]) {
  ...
  // recompile locale to respect new fallbacks
