...
function compile(str) {
  ...
  translator = ["var ctx = this, str = '', nodes;"];
  translator.push("nodes = " + JSON.stringify(nodes) + ";");
  translator.push("params = ctx.flattenParams(params);");
  translator.push("ctx.each(nodes, function (node) {");
  translator.push("  if ('text' === node.type) {");
  translator.push("    str += node.value;");
  translator.push("    return; }");
  translator.push("  if ('variable' === node.type) {");
  translator.push("    str += (params[node.anchor] || '<undefined>').toString();");
  translator.push("    return; }");
  translator.push("  if ('plural' === node.type) {");
  translator.push("    str += ctx.pluralize(+params[node.anchor], node.forms);");
  translator.push("    return; }");
  translator.push("  throw new Error('Unknown node type'); });");
...
BabelFish.prototype.getContext = function getContext(locale) {
  return {
    ...
    pluralize: this.getPluralizer(locale)
  };
};


//  internal
//  BabelFish#getPluralizer(locale) -> Function
//
//  Returns pluralizer function for given locale respecting locale fallbacks.
BabelFish.prototype.getPluralizer = function getPluralizer(locale) {
  // TODO: respect given locale fallbacks while choosing pluralizer
  return (Rules[locale] || Rules[this.defaultLocale] || Rules[GENERIC_LOCALE]).pluralize;
};
